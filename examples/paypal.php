...
$paypalService = $serviceFactory->createService('paypal', $credentials, $storage, array(PayPal::SCOPE_PROFILE, Paypal::SCOPE_OPENID));

if ($paypalService->isGlobalRequestArgumentsPassed()) {
	// Retrieve a token and send a request
	$result = $paypalService->retrieveAccessTokenByGlobReqArgs()->requestJSON('/identity/openidconnect/userinfo/?schema=openid');
