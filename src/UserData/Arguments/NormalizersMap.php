...
class NormalizersMap extends AbstractArgument {
	...
	public function add(array $fields)
	{
		foreach ($fields as $field => $normalizer)
		{
			...
			elseif (0 === strpos($normalizer, '::'))
			{
				...
				if (is_int($field)) $field = $method;
				$this->method($field, $method);
			}
			// 'field' => 'field.path'
			else $this->path($field, $normalizer);
		}
	...
	{
		if (!$methodName) $methodName = $field;

		if (!is_string($field)) throw new GenericException('Must be a string!');
		if (!is_string($methodName)) throw new GenericException('Must be a string!');
	...
	{
		if (!is_string($field)) throw new GenericException('Must be a string!');
		if (!is_string($path)) throw new GenericException('Must be not empty string!');
	...
	{
		if (!empty($this->fields[ $field ])) return $this->fields[ $field ];
		else return FALSE;
	}
	...
	{
		if (!is_string($pathContext)) throw new GenericException('Must be a string!');
