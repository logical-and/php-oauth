...
class LoadersMap extends AbstractArgument {
	...
	{
		if (!is_string($loader)) throw new GenericException('Must be string!');
	...
	public function addField($field)
	{
		...
		if (!is_string($field)) throw new GenericException('Must be string!');
	...
	{
		foreach ($fields as $field) $this->readdField($field);
	...
	public function getLoaderForField($searchField)
	{
		foreach ($this->loaders as $loader => $fields)
		{
			if (in_array($searchField, $fields, TRUE)) return $loader;
		}
	...
	{
		if (!$this->contextLoader) throw new GenericException('Context loader is not defined!');

		if (!isset($this->loaders[ $this->contextLoader ])) $this->loaders[ $this->contextLoader ] = [];
	}
}
